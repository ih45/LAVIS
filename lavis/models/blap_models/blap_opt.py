...
from lavis.models.blap_models.blap import BlapBase, disabled_train
# from lavis.models.blip2_models.modeling_opt import OPTForCausalLM, OPTConfig
from transformers import AutoTokenizer, OPTForCausalLM, OPTConfig
import transformers


@registry.register_model("blap_opt")
class BlapOPT(BlapBase):
    """
    BLIP2 OPT model.
    ...
    def __init__(
        ...
        self.Qformer, self.query_tokens = self.init_Qformer(
            num_query_token, self.audio_encoder.num_features
        )
    ...
    def forward(self, samples):
        ...
        atts_opt = torch.ones(inputs_opt.size()[:-1], dtype=torch.long).to(waveform.device)
        ...
        empty_targets = (
            torch.ones(atts_opt.size(), dtype=torch.long).to(waveform.device).fill_(-100)
        )
