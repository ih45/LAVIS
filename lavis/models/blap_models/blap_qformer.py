...
from lavis.models.blap_models.blap import (
    BlapBase,
    compute_sim_matrix,
    disabled_train
)
...
@registry.register_model("blap")
@registry.register_model("blap_feature_extractor")
class BlapQformer(BlapBase):
    """
    BLAP first-stage model with Q-former and HTSAT.
    Usage:
    ...
    def __init__(
        ...
        self.Qformer, self.query_tokens = self.init_Qformer(
            num_query_token, self.audio_encoder.num_features, cross_attention_freq
        )
    ...
    def generate(
        ...
        input_ids = (
            torch.LongTensor(waveform.size(0), 1)
    ...
    def compute_itm(self, audio_inputs, text_ids, text_atts):
        audio_atts = torch.ones(audio_inputs.size()[:-1], dtype=torch.long).to(
            audio_inputs.device
        )
    ...
    def extract_features(self, samples, mode="multimodal"):
        ...
        assert mode in [
            ...
            "text",
            "multimodal",
        ], "mode must be one of 'waveform', 'text', 'multimodal'"
        ...
        return BlipOutputFeatures(
            ...
            image_embeds_proj=audio_features,
