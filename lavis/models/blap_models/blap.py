...
from lavis.models.blap_models.Qformer import BertConfig, BertLMHeadModel
from lavis.models.eva_vit import create_eva_vit_g
from lavis.models.clip_vit import create_clip_vit_L
from transformers import BertTokenizer


class BlapBase(BaseModel):
    @classmethod
    ...
    # TODO: 修改变量名
    @classmethod
    def init_Qformer(cls, num_query_token, audio_width, cross_attention_freq=2):
...
def compute_sim_matrix(model, data_loader, **kwargs):
    ...
    for samples in data_loader:
        ...
        audio_feat, hsast_feat = model.forward_audio(waveform)
        audio_embed = model.audio_proj(audio_feat)
        audio_embed = F.normalize(audio_embed, dim=-1)

        hsast_feats.append(hsast_feat.cpu())
        audio_embeds.append(audio_embed)

    hsast_feats = torch.cat(hsast_feats, dim=0)
    audio_embeds = torch.cat(audio_embeds, dim=0)

    sims_matrix = []
    for audio_embed in audio_embeds:
        sim_q2t = audio_embed @ text_embeds.t()
    ...
    for i, sims in enumerate(
        ...
        audio_inputs = hsast_feats[start + i].repeat(k_test, 1, 1).to(model.device)
    ...
    for i, sims in enumerate(
        ...
        topk_sim, topk_idx = sims.topk(k=k_test, dim=0)
        audio_inputs = hsast_feats[topk_idx.cpu()].to(model.device)
        score = model.compute_itm(
            audio_inputs=audio_inputs,
